...
data <- sample_n(kidney,300,replace = T)
data <- merge(kidney,data)
data$time <- data$time + rnorm(length(data$time))
data$age <- data$age + rpois(length(data$time),lambda = 0.5)
...
mvQuad::rescale(model_data$thetagrid,domain = c(-4,2))
...
#Ploting:
ggsave(filename = "~/BOOTKidney_PosterTheta.pdf",plot = thetapostplot1)
ggsave(filename = "~/BOOTKidney_PosterSigma.pdf", plot = sigmapostplot1)
...
library(gridExtra)
pdf("BOOTRandomEffects.pdf", height=11, width=8.5)
...
pdf("BOOTFixedEffectsMean.pdf", height=11, width=8.5)
grid.table(table_for_fixed)
dev.off()


pdf("BOOTFixedEffectsSD.pdf", height=11, width=8.5)
