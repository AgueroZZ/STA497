...
source("~/Desktop/STA497/current work/code/1. function for PH Model.R")
options(mc.cores = parallel::detectCores())

randomseed <- 12346
set.seed(randomseed)
N <- 600 # Number of subjects

# Simulate the design matrix:
Xlist <- cbind(seq(from = -20, to = 20, length.out = N))
...
thetagrid <- as.list(seq(-4,3,by = 0.5)) # This is the log(precision)
control1 <- list(
  prec = 1e-06,
