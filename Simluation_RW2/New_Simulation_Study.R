...
Xlist <- cbind(seq(from = 1, to = 20, length.out = N))
truefunc <- function(x) 0.03*(1.8*x^2 + 0.7*sin(0.1*x))
...
data <- data %>% as_tibble() %>%
  mutate(exposure_binned = bin_covariate(exposure,bins = RW2BINS,type = "quantile",decimals = 5))
...
thetagrid <- as.list(seq(-1.5,1.5,by = 0.5)) # This is the log(precision)
control1 <- list(
  prec = 1e-06,
...
node1 <- model_data$vectorofcolumnstoremove
