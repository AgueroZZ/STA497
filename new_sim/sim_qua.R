...
save(sim1opt,file = "~/STA497/new_sim/Optim.Rdata")
...
save(margmeans_and_vars,file = "~/STA497/new_sim/PosteriorMeanVar.Rdata")
...
simplot <- tibble(
  ...
  labs(title = "Fixed Effect vs Fixed Effect plus Smooth Term curve",
       subtitle = "Red = Fixed Effect; Orange = Fixed Effect + Smooth Term; Blue = True function",
       x = "Covariate", y = "eta") +
  theme(text = element_text(size = PLOT_TEXT_SIZE))

ggsave(filename = "~/STA497/new_sim/Proposed.pdf",
       plot = simplot,
       width = 3,
       height = 3.5)
...
ggsave(filename = "~/STA497/new_sim/INLAplot.pdf", plot = new_compare)
