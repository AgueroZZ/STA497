...
#Function that compute the value of the gradient of log-likelihood at a specific value of W.
...
#W is the values of latent field that we evaluate.
# For a 100 data with approximately 40% censoring:
#              user  system elapsed 
#            55.252   0.492  55.796
#Note that it uses a very big storage for the n of n*n matrices.
...
#longer computation time. For a 100 data with approximately 40% censoring:
#             user  system elapsed 
#           57.904   0.601  58.980 
hessian_log_likelihood_alternative <- function(W,model_data) {
  Risk <- Get_all_Risk_Set(model_data)
  ob <- Get_Observed_index(model_data)
  delta <- prep_data_for_log_lik(W,model_data)
  #Temporally fill in Delta_11 into delta, to make the future computation easier.
...
hessian_log_posterior_W <- function(W,theta = NULL,Q = NULL,model_data) {
  ...
  as(-(Q + hessian_log_likelihood(W,model_data)),"dgCMatrix")
}
...
#This one will not be used for Coxph model, just ignore.
compute_marginal_variances1 <- function(i,model_results,model_data,constrA = NULL,correction = -Inf) {
  # W: vector of values of W to compute log pi(W_i|y) at
  # i: index. which element of the latent field do you want the marginal for?
  # model_results: tibble containing output of optimization
